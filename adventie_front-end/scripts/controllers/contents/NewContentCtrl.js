...
angular.module('wain013', ['ngSanitize', 'ui.select', 'summernote', 'ngDropzone', 'daterangepicker', 'siyfion.sfTypeahead'])
    // Chart controller 
    .controller('NewContentCtrl', ['$scope', '$rootScope', '$sce', '$compile', 'LanguageService', 'ArticleService', 'ContentService', '$location','ipCookie',
                                 function($scope, $rootScope, $sce, $compile, LanguageService, ArticleService, ContentService, $location,ipCookie) {
    ...
    $scope.create = function ($redirect) {
        ...
        if(!hasErrorFlag)
         var newArticle = ContentService.save($scope.story, function () {
            
            if($redirect){
                $location.path('/contents/published');
            }else{
