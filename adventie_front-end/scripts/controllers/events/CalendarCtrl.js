...
angular.module('wain013', ['ngPhotoswipe'])
    .controller('CalendarCtrl', ['$scope', '$stateParams', '$rootScope', 'EventsService', function($scope, $stateParams, $rootScope, EventsService) {
        ...
        var events = EventsService.query();
        $scope.events = events;
        console.log(events);
        ...
        $scope.expand = function(key, event) {
            
            console.log(event);
            //console.log($this);
            
            $("#event-"+key+".row-details").toggle();
            $("#event-"+key).toggleClass("shown");
