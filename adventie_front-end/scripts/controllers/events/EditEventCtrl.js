...
// Chart controller
    .controller('EditEventCtrl', ['$scope', '$rootScope', '$sce', '$compile', 'lookupFilter', 'LanguageService', 'EventsService', '$location', 'ipCookie', 'NgMap', '$stateParams','$state',
        function ($scope, $rootScope, $sce, $compile, lookupFilter, LanguageService, EventsService, $location, ipCookie, NgMap, $stateParams,$state) {
            ...
            if($rootScope.temp_event){
                $scope.event = $rootScope.temp_event;
            }else{
                EventsService.get({id: $stateParams.eventId},function (response) {
                    $rootScope.temp_event = response;
                    $state.reload();
                });
            }
            
            $scope.removeImg = function (index) {
                $scope.event.photos.splice(index, 1)
            }

            $scope.tags = $scope.event.tags.join(",");
            ...
            if($scope.event.recurrence){
                if($scope.event.recurrence.length>0){
                    $scope.isRepeated = "no";
                    $scope.selectedDays = $scope.event.recurrence[0].repeat;
                }else{
                    $scope.isRepeated = "yes";
                }
            }else{
                $scope.isRepeated = "yes";
            }
            ...
            for (var i = 0; i < $scope.event.socialMedias.length; i++) {
                if ($scope.event.socialMedias[i].type == "instagram")
                    $scope.instagram = $scope.event.socialMedias[i].url.replace("http://www.instagram.com/", "");

                if ($scope.event.socialMedias[i].type == "twitter")
                    $scope.twitter = $scope.event.socialMedias[i].url.replace("http://www.twitter.com/@", "");

                if ($scope.event.socialMedias[i].type == "facebook")
                    $scope.facebook = $scope.event.socialMedias[i].url.replace("http://www.facebook.com/", "");

                if ($scope.event.socialMedias[i].type == "googlePlus")
                    $scope.googlePlus = $scope.event.socialMedias[i].url.replace("http://www.plus.google.com/", "");

                if ($scope.event.socialMedias[i].type == "website")
                    $scope.website = $scope.event.socialMedias[i].url;

                if ($scope.event.socialMedias[i].type == "snapchat")
                    $scope.snapchat = $scope.event.socialMedias[i].url.replace("https://www.snapchat.com/add/", "");

            }
            $scope.selectedCategory = $scope.event.categories;
            $scope.selectedOption = $scope.event.options;

            $scope.date = {};
            $scope.date.dateEnd = moment($scope.event.endDate).format("MM/DD/YYYY");
            $scope.date.dateStart = moment($scope.event.startDate).format("MM/DD/YYYY");

            $scope.date.timeEnd = moment($scope.event.endDate).format("h:mm A");
            $scope.date.timeStart = moment($scope.event.startDate).format("h:mm A");
                ...
                $scope.create = function ($redirect) {
                    ...
                    for (var i = 0; i < images.length; i++) {
                        ...
                        if (image.data.photo) {
                            $scope.event.photos.push(image.data.photo);
                        }

                    }
                    /*if (eventimages.length > 0)
                        $scope.event.photos.push(eventimages);*/
                    ...
                    if (!hasErrorFlag)
                        var newEvent = EventsService.update({id: $scope.event._id},$scope.event, function () {
/*
                            ...
                            function (err) {
                                ...
                                $('body').pgNotification({
                                    style: 'bar',
                                    message: "Couldn't update the event : " + err.data.message + " :(",
                                    position: 'top', //'bottom'
                                    timeout: 5000,
                                    type: 'error'
                                }).show();
            ...
            $scope.checkCategory = function checkCategory(category) {
                console.log(category);
                var categorya =  angular.toJson(category);
                var categoryaa =  angular.toJson(JSON.parse(categorya));
                console.log(categoryaa);
                console.log($scope.selectedCategory.indexOf(category) > -1);

                return $scope.selectedCategory.indexOf(category) > -1;
            };
